...
HashTable::HashTable() {
  ...
  memset(E,0,sizeof(E));
  kHasCollisionRaw = false;
  kHashBucket = 0;
  memset(&kXExisting,0,sizeof(kXExisting));
  memset(&kDExisting,0,sizeof(kDExisting));
  memset(&kXIncoming,0,sizeof(kXIncoming));
  memset(&kDIncoming,0,sizeof(kDIncoming));
...
ENTRY *HashTable::CreateEntry(int128_t *x,int192_t *d) {
...
void HashTable::Convert(Int *x,Int *d,uint32_t type,uint64_t *h,int128_t *X,int192_t *D) {
  ...
  if(d->bits64[3] > 0x7FFFFFFFFFFFFFFFULL) {
    ...
    D->i64[0] = N.bits64[0];
    D->i64[1] = N.bits64[1];
    D->i64[2] = N.bits64[2] & 0x3FFFFFFFFFFFFFFFULL;
    sign = 1ULL << 63;
  } else {
    D->i64[0] = d->bits64[0];
    D->i64[1] = d->bits64[1];
    D->i64[2] = d->bits64[2] & 0x3FFFFFFFFFFFFFFFULL;
  }
  ...
  *nbDP = 0;
  ...
  while(!(end1 && end2)) {
    ...
    if( !end1 && !end2 ) {
      ...
      if(comp < 0) {
        memcpy(output+nbd,&e1,sizeof(ENTRY));
        nbd++;
        AV1();
        nb1--;
      } else if (comp==0) {
        if((e1.d.i64[0] == e2.d.i64[0]) && (e1.d.i64[1] == e2.d.i64[1]) && (e1.d.i64[2] == e2.d.i64[2])) {
        ...
        memcpy(output + nbd,&e2,sizeof(ENTRY));
        nbd++;
        AV2();
        nb2--;
      }
  ...
  ::fwrite(&md,sizeof(uint32_t),1,fd);
  ::fwrite(output,sizeof(ENTRY),nbd,fd);
  free(output);
...
int HashTable::Add(uint64_t h,int128_t *x,int192_t *d) {
...
void HashTable::CalcDistAndType(int192_t d,Int* kDist,uint32_t* kType) {
  ...
  *kType = (d.i64[2] & 0x4000000000000000ULL) != 0;
  int sign = (d.i64[2] & 0x8000000000000000ULL) != 0;
  d.i64[2] &= 0x3FFFFFFFFFFFFFFFULL;
...
int HashTable::Add(uint64_t h,ENTRY* e) {
  ...
  while(st <= ed) {
    ...
    if(comp<0) {
      ...
      if((e->d.i64[0] == GET(h,mi)->d.i64[0]) && (e->d.i64[1] == GET(h,mi)->d.i64[1]) && (e->d.i64[2] == GET(h,mi)->d.i64[2])) {
        // Same point added 2 times or collision in same herd !
        return ADD_DUPLICATE;
      }

      // Collision
      kHasCollisionRaw = true;
      kHashBucket = h;
      kXExisting = GET(h,mi)->x;
      kDExisting = GET(h,mi)->d;
      kXIncoming = e->x;
      kDIncoming = e->d;
...
void HashTable::SeekNbItem(FILE* f,uint32_t from,uint32_t to) {
  ...
  for(uint32_t h = from; h < to; h++) {
    ...
    uint64_t hSize = (uint64_t)sizeof(ENTRY) * E[h].nbItem;
#ifdef WIN64
