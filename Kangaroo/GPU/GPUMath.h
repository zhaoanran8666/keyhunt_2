...
__device__ __constant__ uint64_t DIST_SIGN_BIT = 0x8000000000000000ULL;
#endif
...
__device__ __forceinline__ void DistAddSigned128(uint64_t *r,const uint64_t *a) {

  uint64_t sign = r[1] & DIST_SIGN_BIT;
  uint64_t mag0 = r[0];
  uint64_t mag1 = r[1] & ~DIST_SIGN_BIT;

  if(sign == 0ULL) {
    // Positive distance: |d| += jump
    uint64_t lo = mag0 + a[0];
    uint64_t carry = (lo < mag0) ? 1ULL : 0ULL;
    mag0 = lo;
    mag1 = mag1 + a[1] + carry;
  } else {
    // Negative distance: |d| -= jump, sign flips when |d| < jump
    bool magGeJump = (mag1 > a[1]) || (mag1 == a[1] && mag0 >= a[0]);
    if(magGeJump) {
      uint64_t borrow = (mag0 < a[0]) ? 1ULL : 0ULL;
      mag0 = mag0 - a[0];
      mag1 = mag1 - a[1] - borrow;
    } else {
      uint64_t borrow = (a[0] < mag0) ? 1ULL : 0ULL;
      mag0 = a[0] - mag0;
      mag1 = a[1] - mag1 - borrow;
      sign = 0ULL;
    }
  }
...
__device__ __forceinline__ void DistToggleSign128(uint64_t *r) {
  if((r[0] | (r[1] & ~DIST_SIGN_BIT)) == 0ULL) {
    r[1] = 0ULL;
  } else {
    r[1] ^= DIST_SIGN_BIT;
  }
