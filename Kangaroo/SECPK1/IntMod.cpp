...
void Int::DivStep62(Int* u,Int* v,int64_t* eta,int* pos,int64_t* uu,int64_t* uv,int64_t* vu,int64_t* vv) {
  ...
  // Former divstep62 — 已禁用，仅保留参考
  // 此段使用 SSE2 __m128i，在 ARM64 上不可用
  ...
  // ARM64 兼容：使用两个 int64_t 变量替代 __m128i 向量
  // _u 存储 (uu, uv)，_v 存储 (vu, vv)
  int64_t _u0 = 1, _u1 = 0;  // uu=1, uv=0
  int64_t _v0 = 0, _v1 = 1;  // vu=0, vv=1
  ...
  while(true) {
    ...
    vh >>= zeros;
    v0 >>= zeros;
    // 等效于 _mm_slli_epi64(_u, zeros)：两个分量同时左移
    _u0 <<= zeros;
    _u1 <<= zeros;
    bitCount -= (int)zeros;
