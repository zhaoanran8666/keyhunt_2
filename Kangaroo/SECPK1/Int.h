...
#ifdef __aarch64__
// ===== ARM64 (Apple Silicon / AArch64) 兼容实现 =====
// 使用 __int128 替代 x86 内联汇编，Clang 会生成高效的 MUL/UMULH 指令对
...
static int64_t inline _mul128(int64_t a, int64_t b, int64_t *h) {
  ...
  *h = (int64_t)((uint64_t)((unsigned __int128)r >> 64));
  return (int64_t)(uint64_t)r;
}

// 128÷64 位除法（ARM64 上使用 __uint128_t 模拟）
...
// 带进位加法：result = a + b + carry_in，返回 carry_out
static unsigned char inline _addcarry_u64(unsigned char c_in, uint64_t a, uint64_t b, uint64_t *result) {
  __uint128_t sum = (__uint128_t)a + b + c_in;
  *result = (uint64_t)sum;
  return (unsigned char)(sum >> 64);
}

// 带借位减法：result = a - b - borrow_in，返回 borrow_out
static unsigned char inline _subborrow_u64(unsigned char b_in, uint64_t a, uint64_t b, uint64_t *result) {
  __uint128_t sub = (__uint128_t)a - b - b_in;
  *result = (uint64_t)sub;
  // 如果发生了借位，高 64 位会是全 1（二进制补码）
