...
#ifdef USE_SYMMETRY
    // In symmetry mode, wild entries can map to equivalent points against either
    // keyToSearch or keyToSearchNeg. Accept both branches when validating.
    if(!ok && types[i] == WILD) {
      Point alt = secp->AddDirect(keyToSearchNeg, P[i]);
      uint32_t hAlt = alt.x.bits64[2] & HASH_MASK;
      ok = (hAlt == h) && (alt.x.bits64[0] == e->x.i64[0]) && (alt.x.bits64[1] == e->x.i64[1]);
    }
#endif
    if(!ok) nbWrong++;
    //if(!ok) {
...
void Kangaroo::Check(std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  */
  ...
  if(useGpu) {
...
#ifdef WITHMETAL
    ::printf("Metal unit tests...");
    if(!GPUEngine::RunUnitTests(gpuId[0],512)) {
      ::printf(" failed\n");
      return;
    }
    ::printf(" ok\n");
    ...
    int nb = h.GetNbThread() * h.GetGroupSize();
