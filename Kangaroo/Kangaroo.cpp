...
static string Hex128(const int128_t &v) {
  char out[33];
  ::snprintf(out,sizeof(out),"%016" PRIX64 "%016" PRIX64,v.i64[1],v.i64[0]);
  return string(out);
}
...
Kangaroo::Kangaroo(Secp256K1 *secp,int32_t initDPSize,bool useGpu,string &workFile,string &iWorkFile,uint32_t savePeriod,bool saveKangaroo,bool saveKangarooByServer,
  ...
  this->saveRequest.store(false,std::memory_order_relaxed);
  this->connectedClient = 0;
  this->totalRW = 0;
  this->collisionInSameHerd = 0;
  this->unexpectedCollisionDiagPrinted = false;
  this->unexpectedCollisionGpuDiagPrinted = false;
  this->lastCollisionWasUnexpectedWrong = false;
  this->lastAddFromGpu = false;
...
bool Kangaroo::CollisionCheck(Int* d1,uint32_t type1,Int* d2,uint32_t type2) {
  ...
  lastCollisionWasUnexpectedWrong = false;
  ...
  if(type1 == type2) {
    ...
    if(!endOfSearch) {
      ...
      lastCollisionWasUnexpectedWrong = true;
      ...
      if(!unexpectedCollisionDiagPrinted) {
        unexpectedCollisionDiagPrinted = true;
        ::printf("UnexpectedCollisionDiag: type1=%u type2=%u\n",type1,type2);
        if(hashTable.kHasCollisionRaw) {
          ::printf("UnexpectedCollisionDiag: hashBucket=%06" PRIX64 "\n",hashTable.kHashBucket);
...
void Kangaroo::SolveKeyCPU(TH_PARAM *ph) {
  ...
  while(!endOfSearch) {
    ...
    // Save request
    if(saveRequest.load(std::memory_order_acquire) && !endOfSearch) {
      ph->isWaiting = true;
      LOCK(saveMutex);
      ph->isWaiting = false;
      UNLOCK(saveMutex);
...
void Kangaroo::SolveKeyGPU(TH_PARAM *ph) {
  ...
  ph->nbKangaroo = (uint64_t)gpu->GetNbThread() * (uint64_t)gpu->GetGroupSize();
  ...
  if( ph->px==NULL ) {
    ...
    uint64_t nbThreadPerGroup = (uint64_t)gpu->GetThreadsPerGroup();
    int gpuGroupSize = gpu->GetGroupSize();
    ph->px = new Int[ph->nbKangaroo];
    ph->py = new Int[ph->nbKangaroo];
    ph->distance = new Int[ph->nbKangaroo];

    // Keep initial herd type aligned with kIdx%2 (used by GPU DP decode/collision path).
    // GPU memory layout and kIdx are [block][g][t], where t parity defines herd type.
    if(nbThreadPerGroup == 0 || (nbThread % nbThreadPerGroup) != 0) {
      // Fallback to legacy linear init if thread geometry is unexpected.
      for(uint64_t i = 0; i < nbThread; i++) {
        CreateHerd(gpuGroupSize,&(ph->px[i * gpuGroupSize]),
                                &(ph->py[i * gpuGroupSize]),
                                &(ph->distance[i * gpuGroupSize]),
                                TAME);
      }
    } else {
      uint64_t nbThreadGroup = nbThread / nbThreadPerGroup;
      uint64_t walkersPerBlock = nbThreadPerGroup * (uint64_t)gpuGroupSize;
      for(uint64_t b = 0; b < nbThreadGroup; b++) {
        uint64_t blockBase = b * walkersPerBlock;
        for(int g = 0; g < gpuGroupSize; g++) {
          uint64_t base = blockBase + (uint64_t)g * nbThreadPerGroup;
          CreateHerd((int)nbThreadPerGroup,
                     &(ph->px[base]),
                     &(ph->py[base]),
                     &(ph->distance[base]),
                     TAME);
  ...
  if(!gpu->callKernel()) {
    ::printf("SolveKeyGPU Thread GPU#%d: initial kernel launch failed\n",ph->gpuId);
    ph->hasStarted = true;
    endOfSearch = true;
    safe_delete_array(ph->px);
    safe_delete_array(ph->py);
  ...
  while(!endOfSearch) {
    ...
    if(!gpu->Launch(gpuFound)) {
      ::printf("\nSolveKeyGPU Thread GPU#%d: kernel execution failed, stopping GPU thread\n",ph->gpuId);
      endOfSearch = true;
      break;
    }
    counters[thId] += (uint64_t)gpu->GetNbThread() * (uint64_t)gpu->GetGroupSize() *
                      (uint64_t)gpu->GetRunCount();
        ...
        for(int g = 0; !endOfSearch && g < gpuFound.size(); g++) {
          ...
          if(!AddToTable(&gpuFound[g].x,&gpuFound[g].d,kType)) {
            ...
            if(lastCollisionWasUnexpectedWrong && !unexpectedCollisionGpuDiagPrinted) {
              ...
              if(hasSymClass) {
                ::printf("UnexpectedCollisionDiagGPU: kIdx=%" PRIu64 " symClass=%" PRIu64
                         " stateCache=%s(%d) nbRun=%d\n",
                         gpuFound[g].kIdx,
                         symClass & 1ULL,
                ...
                ::printf("UnexpectedCollisionDiagGPU: kIdx=%" PRIu64
                         " symClass=NA stateCache=%s(%d) nbRun=%d\n",
                         gpuFound[g].kIdx,
                         StateCacheModeName(lastAddGpuStateCacheMode),
                         lastAddGpuStateCacheMode,
                         lastAddGpuRunCount);
              }
              unexpectedCollisionGpuDiagPrinted = true;
            }
#endif
            // Collision inside the same herd
            // We need to reset the kangaroo
...
void Kangaroo::Run(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  // Compute grid size
  int gpuGroupSize = GPUEngine::GetDefaultGroupSize();
